...
# Instancia o app
app = DjangoDash("CambioApp", suppress_callback_exceptions=True)
...
nome_legenda = {
    ...
    "assessores_ativos": "Assessores Ativos"
}
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Câmbio", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("cambio-graph", "figure"),
    [
        Input("cambio-nome", "value"),
        Input("cambio-ano", "value"),
        Input("cambio-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Câmbio - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
