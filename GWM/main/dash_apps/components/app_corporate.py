...
# Instancia o app
app = DjangoDash("CorporateApp", suppress_callback_exceptions=True)
...
app.layout = html.Div([
    html.H3("Comparativo Corporate", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("corporate-graph", "figure"),
    [
        Input("corporate-nome", "value"),
        Input("corporate-ano", "value"),
        Input("corporate-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Corporate - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
