...
app = create_evolution_app(
    app_name="EvolucaoVolumeApp",
    metric_name="volume",
    title="Evolução de Volume Financeiro",
    y_axis_title="Volume Financeiro (R$)"
)
...
# Layout do app
app.layout = html.Div([
    html.H3("Evolução de Volume Financeiro", style={"textAlign": "center", "color": "black"}),

    html.Div([
        html.Div([
            dcc.Dropdown(
                id="EvolucaoVolumeApp-ano",
...
@app.callback(
    Output("EvolucaoVolumeApp-graph", "figure"),
    [
        Input("EvolucaoVolumeApp-ano", "value"),
        Input("EvolucaoVolumeApp-meses", "value")
    ]
)
def atualizar_grafico(ano, intervalo_meses):
    ...
    ][["year", "month", "volume"]].rename(columns={"volume": "planejado"})
    ...
    ][["year", "month", "volume"]].rename(columns={"volume": "realizado"})
    ...
    fig.update_layout(
        title=dict(
            text=f"Evolução de Volume Financeiro<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
