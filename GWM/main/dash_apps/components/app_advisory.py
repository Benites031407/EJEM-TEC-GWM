...
# Instancia o app
app = DjangoDash("AdvisoryApp", suppress_callback_exceptions=True)
...
# Nome amigável para os filtros
nome_legenda = {
    "pl_liquidez": "PL em Liquidez",
    "percentual_pl_liquidez": "% PL em Liquidez",
    "volume_credito": "Volume Crédito",
    "percentual_pl_credito": "% PL Crédito",
    "ofertas_publicas": "Ofertas Públicas"
}
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Advisory", style={"textAlign": "center", "color": "black"}),

    html.Div([
        html.Div([
            dcc.Dropdown(
                id="advisory-nome",
                options=[{"label": nome_legenda.get(n, n), "value": n} for n in nomes],
...
@app.callback(
    Output("advisory-graph", "figure"),
    [
        Input("advisory-nome", "value"),
        Input("advisory-ano", "value"),
        Input("advisory-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    if not nome_tipo or not ano or not intervalo_meses:
    ...
    ][["year", "month", nome_tipo]].rename(columns={nome_tipo: "planejado"})
    ...
    ][["year", "month", nome_tipo]].rename(columns={nome_tipo: "realizado"})
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Advisory - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
