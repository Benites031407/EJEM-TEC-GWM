...
# Instancia o app
app = DjangoDash("ConsorcioApp", suppress_callback_exceptions=True)
...
nome_legenda = {
    "qtd_reunioes": "Reuniões",
    "qtd_consorcios": "Consórcios",
...
nomes = ["qtd_reunioes", "qtd_consorcios", "volume_financeiro"]
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Consórcio", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("consorcio-graph", "figure"),
    [
        Input("consorcio-nome", "value"),
        Input("consorcio-ano", "value"),
        Input("consorcio-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Consórcio - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
