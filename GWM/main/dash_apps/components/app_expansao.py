...
# Instancia o app
app = DjangoDash("ExpansaoApp", suppress_callback_exceptions=True)
...
nomes = ["nps", "contratacoes", "entrevistas"]
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Expansão", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("expansao-graph", "figure"),
    [
        Input("expansao-nome", "value"),
        Input("expansao-ano", "value"),
        Input("expansao-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Expansão - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
