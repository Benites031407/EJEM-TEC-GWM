...
def create_evolution_app(app_name, metric_name, title, y_axis_title):
    # Instancia o app
    app = DjangoDash(app_name, suppress_callback_exceptions=True)
    ...
    # Layout do app
    app.layout = html.Div([
        html.H3(title, style={"textAlign": "center", "color": "black"}),
    ...
    @app.callback(
        Output(f"{app_name}-graph", "figure"),
        [
            Input(f"{app_name}-ano", "value"),
            Input(f"{app_name}-meses", "value")
        ]
    )
    def atualizar_grafico(ano, intervalo_meses):
        ...
        ][["year", "month", metric_name]].rename(columns={metric_name: "planejado"})
        ...
        ][["year", "month", metric_name]].rename(columns={metric_name: "realizado"})
        ...
        fig.update_layout(
            title=dict(
                text=f"{title}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
                x=0.5,
                xanchor="center",
