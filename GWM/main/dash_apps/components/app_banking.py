...
# Instancia o app
app = DjangoDash("BankingApp", suppress_callback_exceptions=True)
...
nome_legenda = {
    ...
    "cartoes_emitidos": "Cartões Emitidos"
}
...
nomes = ["principalidade", "cartoes_emitidos"]
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Banking", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("banking-graph", "figure"),
    [
        Input("banking-nome", "value"),
        Input("banking-ano", "value"),
        Input("banking-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Banking - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
