...
# Instancia o app
app = DjangoDash("MarketingApp", suppress_callback_exceptions=True)
...
nome_legenda = {
    ...
    "leads_sociais": "Leads Sociais",
    "percentual_pl_credito": "% PL Crédito",
    "captacao_mesa": "Captação Mesa"
}
...
nomes = ["seguidores", "interacoes", "leads_sociais", "percentual_pl_credito", "captacao_mesa"]
...
# Layout do app
app.layout = html.Div([
    html.H3("Comparativo Marketing", style={"textAlign": "center", "color": "black"}),
...
@app.callback(
    Output("marketing-graph", "figure"),
    [
        Input("marketing-nome", "value"),
        Input("marketing-ano", "value"),
        Input("marketing-meses", "value")
    ]
)
def atualizar_grafico(nome_tipo, ano, intervalo_meses):
    ...
    fig.update_layout(
        title=dict(
            text=f"Comparativo Marketing - {nome_legenda.get(nome_tipo, nome_tipo)}<br><sup>{x_labels[0]} a {x_labels[-1]}/{ano}</sup>",
            x=0.5,
            xanchor="center",
